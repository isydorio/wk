...
procedure TFormClient.sbDeleteClick(Sender: TObject);
begin
regOJSON := TJSONObject.Create;
regOJSON.AddPair('op', 'delete');
regOJSON.AddPair('doc', edtDocumento.Text);
RESTRequest1.Method:=rmPOST;
RESTRequest1.Params[0].Value:=regOJSON.ToString;
...
Memo1.Lines.Add(retOJSON.GetValue<string>('ignores', '')+' Registros ignorados');
for i := 0 to Pred(ComponentCount) do
if Components[i] is TEdit then
  TEdit(Components[i]).Text:='';
cbNatureza.SetFocus;
